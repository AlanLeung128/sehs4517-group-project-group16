<DOCUMENT filename="reservation.blade.php">
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body class="bg-brand-bg text-brand-text font-sans leading-relaxed">
    ...
    <header class="bg-brand-header text-brand-header-text py-8">
        <div class="max-w-6xl mx-auto px-8 flex items-center justify-between">
            ...
            <div class="flex gap-4">
                <a href="/view-today" class="bg-white text-brand-header px-6 py-3 rounded-3xl font-medium hover:bg-amber-100 transition-all">
                    View Today's Reservations
                </a>
                <a href="/logout" class="bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-3xl font-medium transition-all">
                    Logout
                </a>
            </div>
        </div>
    ...
    <div id="bookingErrorModal" class="fixed inset-0 bg-black/70 flex items-center justify-center z-[9999]">
        <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full mx-4 overflow-hidden animate-fade-in">
            <div class="bg-red-600 text-white px-8 py-6 text-center">
                <div class="text-5xl mb-3">🚫</div>
                <h3 class="text-2xl font-bold">Booking Failed</h3>
            </div>
            <div class="p-8 text-center">
                <p class="text-xl text-gray-700 whitespace-pre-line leading-relaxed">
                    {{ session('bookingError') }}
                </p>
            </div>
            <div class="border-t px-8 py-5 flex justify-center">
                <button onclick="document.getElementById('bookingErrorModal').style.display='none'"
                        class="bg-brand-subtitle hover:bg-brand-title text-white font-bold px-10 py-4 rounded-3xl text-lg transition-all">
                    Got it, thanks
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        function renderTimeSlots() {
            ...
            grid.innerHTML = timeSlots.map(t => {
                const isBooked = bookedSlots[selectedSpace] && bookedSlots[selectedSpace].includes(t.value);

                return `
                    <label class="flex flex-col items-center justify-center p-3 border rounded-2xl 
                                  ${isBooked ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'hover:bg-amber-100 cursor-pointer'}">
                        <div class="flex items-center">
                            <input type="checkbox" name="time_slots[]" value="${t.value}" 
                                   class="mr-2" ${isBooked ? 'disabled' : ''}>
                            ${t.display}
                        </div>
                        ${isBooked ? 
                            '<span class="text-xs text-red-600 mt-1">This time slot is already booked by others</span>' 
                            : ''}
                    </label>
                `;
            }).join('');
